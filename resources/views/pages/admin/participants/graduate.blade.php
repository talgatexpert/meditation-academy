...
@section('title', 'Архив выпускников' . ($curator ? ' куратора "' . $curator->name . '"' : ''))
                ...
                <table class="table table-striped table-bordered table-hover order-column">
                    ...
                    @foreach ($participants as $participant)
                        ...
                        <tr class="odd gradeX">
                            ...
                            <td>{{ $participant->getGraduatedDate() }}</td>
                            <td>{{ $participant->certificate }}</td>
            ...
            const participantsTable = $('.table').dataTable({
                ...
                initComplete: function (settings, json) {
                    ...
                    $('.dt-filter-curator select').change(function (e, value) {
                        ...
                        window.location.href = '{{ route('admin.participants.graduates') }}' + (curator !== '' ? '?curator=' + curator : '');
                    });

                    // Инициализируем кнопку очистки для поля с поиском
                    $('.dataTables_filter input').after($('<a href="javascript:void(0);" class="dt-filter-clear" title="Очистить поле"><i class="fa fa-times"></i></a>').on('click', function () {
                        if (participantsTable) {
                            participantsTable.fnFilter('');
                        }
                    }));
